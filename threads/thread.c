...
void thread_set_nice(int nice UNUSED)
{
	...
	struct thread *cur = thread_current();
	cur->nice = nice;		 // nice 값 변경
	mlfqs_priority(cur); // 우선순위 재계산
...
int thread_get_nice(void)
{
	...
	int nice = thread_current()->nice; // 현재 스레드의 nice 값 가져오기

	intr_set_level(old_level); // 인터럽트 복원

	return nice; // nice 값 반환
}
...
int thread_get_load_avg(void)
{
	...
	int load_avg_times_100 = fp_to_int(mult_mixed(load_avg, 100)); // load_avg에 100 곱하기

	intr_set_level(old_level); // 인터럽트 복원

	return load_avg_times_100; // 결과 반환
}
...
int thread_get_recent_cpu(void)
{
	...
	// int recent_cpu_times_100 = fp_to_int_round(mult_mixed(thread_current()->recent_cpu, 100)); // recent_cpu에 100 곱하기
	int recent_cpu_times_100 = fp_to_int(mult_mixed(thread_current()->recent_cpu, 100)); // recent_cpu에 100 곱하기
...
{
	if (t == idle_thread)
		return; // Idle thread는 계산에서 제외

	// 고정소수점 연산을 사용하여 우선순위 계산
	int new_priority = int_to_fp(PRI_MAX);
	new_priority = sub_fp(new_priority, div_mixed(t->recent_cpu, 4));
	new_priority = sub_fp(new_priority, int_to_fp(t->nice * 2));

	// 계산된 우선순위를 정수형으로 변환하고 범위 내로 제한
	t->priority = fp_to_int_round(new_priority);
...
void mlfqs_load_avg(void)
{
	...
	{
		ready_threads++; // 현재 실행 중인 스레드도 포함
	}

	// 고정소수점 연산을 사용하여 load_avg 계산
	int load_avg_contrib1 = mult_fp(div_fp(int_to_fp(59), int_to_fp(60)), load_avg);
	int load_avg_contrib2 = mult_fp(div_fp(int_to_fp(1), int_to_fp(60)), int_to_fp(ready_threads));

	load_avg = add_fp(load_avg_contrib1, load_avg_contrib2);

	// load_avg가 0보다 작아지지 않도록 보장
	if (load_avg < 0)
		load_avg = 0;
}
...
void mlfqs_increment(void)
{
	...
	if (current_thread != idle_thread)
	{
		...
		current_thread->recent_cpu = add_mixed(current_thread->recent_cpu, 1);
	}
}
